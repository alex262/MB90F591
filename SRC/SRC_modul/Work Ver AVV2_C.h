...
#ifdef PLATA_AVV2_M1
#define NAME_MODUL "AVV2-M1"
#else
#define NAME_MODUL "AVV2C"
#endif
...
typedef struct
{
	...
	WORD	DAC;			//Значение записываемое в ЦАП
	WORD	NewDAC;			//Новое значение записываемое в ЦАП
	
	//DWORD   	rADC[6];	//Значение кодов считываемое с 6 каналов (??-Не нужен)
	WORD 		Timer;		//Таймер	
	...
	BYTE 	workN_CH_AVV;	//Читаемый в данный момент канал АВВ
	BYTE 	newN_CH_AVV;	//Новый канал предназначенный для чтения
